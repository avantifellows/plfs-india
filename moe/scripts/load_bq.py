#!/usr/bin/env python3
"""
Load board-results clean parquet from GCS into BigQuery.

Each table listed in sources.py is loaded via load_table_from_uri with
...
Pre-reqs (one-time):
  ...
  (Parquet files must already be in gs://avantifellows-external-data/board_results/clean/
   — see scripts/upload_to_gcs.py.)

Usage:
  python3 scripts/load_bq.py                                  # load all tables
  python3 scripts/load_bq.py --table board_results_fact_overall
...
def main() -> None:
    ...
    print(f"board_results → {BQ_PROJECT}.external_data_sources.*   ({'dry-run' if args.dry_run else 'load'})")
    for t in chosen:
