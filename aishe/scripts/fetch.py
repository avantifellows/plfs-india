#!/usr/bin/env python3
"""
Fetch the raw AISHE Final Report workbooks from their canonical source URLs
into raw/.

Makes the source files regenerable from scratch — no manual download. URLs are
in sources.py (REPORT_URLS). After fetching, run clean_aishe.py →
...
Usage:
  ...
  python3 scripts/fetch.py --year 2021-22     # one report
"""
...
def _download(url: str, dest: Path) -> None:
    ...
    with urllib.request.urlopen(req, timeout=120) as resp, open(dest, "wb") as f:
        f.write(resp.read())
    print(f"  ✓ {dest.name}  ({dest.stat().st_size / 1e6:.2f} MB)  ← {url}")


def main() -> None:
    ap = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    ap.add_argument("--year", default=None, help="Fetch only this report year (e.g. 2021-22)")
    ...
    for year in years:
        if year not in REPORT_URLS:
            raise SystemExit(f"unknown year {year!r}; known: {list(REPORT_URLS)}")
        dest = REPORTS[year]
