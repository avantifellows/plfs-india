...
def main() -> None:
    ...
    print(f"nas_fact_national_proficiency: {len(df):,} rows → {out.name}")
    print(f"  grades={sorted(df.grade.unique())}  subjects={df.subject.nunique()}  "
          f"dimensions={df.dimension.nunique()}")
